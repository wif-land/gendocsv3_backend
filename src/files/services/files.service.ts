...
export class FilesService {
  ...
  async compileSections(dto: CreateRecopilationDto) {
    const result = await this.gcpService.compileSections(
      dto.documentsInfo,
      dto.templateId,
    )

    if (!result) {
      throw new HttpException('Error compiling sections', HttpStatus.CONFLICT)
    }
